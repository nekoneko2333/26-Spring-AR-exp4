...
public static class ARBookGameShellSetup
{
    ...
    private static int BindShellUi(ARBookGameShellController controller)
    {
        ...
        if (controller.hudRoot != null)
        {
            ...
            Button returnHome = EnsureButton(
                ...
                new Vector2(1f, 0f),
                new Vector2(1f, 0f));
            ApplyCompanionReturnHomeButtonPresentation(returnHome);
        ...
        bool updateExistingRect = true)
    {
        ...
        if (existing != null)
        {
            if (updateExistingRect)
            {
                ApplyButtonRect(existing, anchoredPosition, size, anchorMin, anchorMax);
            }
        ...
        ApplyButtonRect(rect, anchoredPosition, size, anchorMin, anchorMax);
    ...
    private static void ApplyButtonRect(
        Button button,
        Vector2 anchoredPosition,
        Vector2 size,
        Vector2 anchorMin,
    ...
    private static void ApplyCompanionReturnHomeButtonPresentation(Button button)
    {
        if (button == null)
        {
            return;
        ...
        Undo.RecordObject(text, "Set Companion Return Home Button Presentation");
        text.text = "返\n回\n首\n页";
        text.fontSize = 20f;

...
public static class ARBookAudioAutoBinder
{
    ...
    public static void AutoBindAudioCues()
    {
        ...
        changed += BindBattle(attack, hit, chapterEnd, challengeFailure, defaultBgmCue, battleBgmCue);
    ...
    private static int BindObjectiveEvents(ARBookAudioCue chapterEndCue)
    {
        ...
        for (int i = 0; i < objectives.Length; i++)
        {
            ...
            changed += AddPersistentListener(
                objective.onObjectiveCompleted,
                chapterEndCue,
                nameof(ARBookAudioCue.PlayOneShot));
    ...
    private static int BindMovementEvents(
        ARBookAudioCue successCue,
        ARBookAudioCue failureCue)
    {
        int changed = 0;
    ...
    private static int BindActivationEvents(
        ARBookAudioCue stepCue,
        ARBookAudioCue completedCue)
    {
        int changed = 0;
        ...
        ARBookAudioCue battleBgmCue)
    {
        ...
        for (int i = 0; i < combatants.Length; i++)
        {
            ...
            Undo.RecordObject(combatant, "Bind combatant audio");
        ...
        for (int i = 0; i < battles.Length; i++)
        {
            ...
            Undo.RecordObject(battle, "Bind battle audio events");
    ...
    private static int BindPresentationBgm(
        ARBookAudioCue defaultBgmCue,
        ARBookAudioCue battleBgmCue)
    {
        int changed = 0;
    ...
    {
        string[] guids = AssetDatabase.FindAssets($"{clipName} t:AudioClip", new[] { AudioFolder });
    ...
    private static int AddPersistentListener(
    ...
    private static int SetClip(
        AudioClip current,
        AudioClip target,
        System.Action<AudioClip> assign)
    {
    ...
    private static int EnsureAudioSource(
        GameObject owner,
        AudioSource current,
        System.Action<AudioSource> assign)
    {
